...
| @liu2021dvnc (Discrete-Valued Neural Communication, NeurIPS 2021) | Codebook global partagé de type VQ-VAE pour des messages discrets entre modules neuronaux | Art antérieur le plus proche pour la communication discrète inter-modules ; DVNC utilise un seul codebook partagé et des modules homogènes, tandis que notre revendication de portabilité au substrat (DR-3) et le protocole nerve-wml compagnon utilisent des codebooks par substrat avec des transducteurs appris et des rôles prédiction/erreur typés — DVNC est un cas particulier (substrat unique, codebook unique) de la portabilité plus générale que cible notre Critère de Conformité |
| @pedersen2024nir (Neuromorphic Intermediate Representation, Nature Communications 2024) | IR de graphe de calcul statique : un jeu d'instructions agnostique au matériel pour compiler un modèle spiking sur de nombreux backends neuromorphiques | Opère à la couche *graphe de modèle* ; orthogonal à notre framework, qui compose des *opérations de consolidation* sur un substrat et est agnostique à l'IR qui abaisse un substrat donné vers le matériel — NIR pourrait servir de cible de compilation pour notre substrat E-SNN (§9.1) sans modifier les axiomes |
| @aer2025biohybrid (Address-Event Representation over UDP/Ethernet, arXiv:2501.09128, 2025) | Couche de transport / encodage spécifiant comment les événements de spike sont horodatés, adressés et transportés physiquement, y compris les liens biohybrides silicium–wetware | Opère à la couche *transport* ; orthogonal à notre framework — le Critère de Conformité contraint la sémantique des opérations de consolidation, pas le protocole filaire qui transporte les spikes, et AER est un porteur admissible pour un substrat E-SNN conforme |
| @liu2024hybrid (Advancing brain-inspired computing with hybrid neural networks, National Science Review 2024) | Revue soutenant que les systèmes hybrides ANN/SNN sont la voie vers l'informatique bio-inspirée | Ancre canonique réseau-neuronal-hybride pour notre revendication cross-substrat : notre validation cycle-1 sur un substrat MLX dense et un substrat LIF spiking est précisément une configuration de réseau neuronal hybride, et le Critère de Conformité exécutable fournit le contrat dont de tels hybrides manquent actuellement |
...
**Les critiques 2026 et un raffinement global versus local.** La PRH a depuis été contestée. @platoscave2026 rapportent que l'alignement représentationnel inter-modèles est fragile et se dégrade avec la mise à l'échelle des modèles, et @aristotelianprh2026 montrent que la convergence *globale* initialement revendiquée est substantiellement un facteur de confusion largeur/profondeur : une fois l'alignement null-calibré par rapport à des baselines aléatoires de capacité équivalente, seule la structure de *voisinage local* subsiste comme signal genuinement inter-substrats. Nous intégrons cela dans DR-3 plutôt que contre lui. Notre revendication d'agnosticisme au substrat n'exige pas que les substrats MLX, E-SNN et LoRA partagent une représentation globale unique ; elle exige seulement qu'un substrat *conforme* préserve la structure opérationnelle *locale* que vérifie le Critère de Conformité — que les mêmes opérations de consolidation, composées dans le même ordre, produisent des états d'épisode équivalents au sens du voisinage. C'est la forme locale de la PRH, et c'est la forme que les données 2026 soutiennent encore. La sonde nerve-wml compagne [@saillant2026nervewml] est cohérente avec cette lecture : sa mesure mutual-kNN est une métrique de voisinage local rapportée par rapport à une baseline aléatoire de capacité équivalente, exactement la quantité null-calibrée et locale que @aristotelianprh2026 soutiennent être la plus défendable. DR-3 est donc ancré à la convergence représentationnelle *locale*, et non globale.
