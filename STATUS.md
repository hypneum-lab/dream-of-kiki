...
**As of** : 2026-05-03 G5-ter pilot (spiking-CNN ported to E-SNN, 40 cells Option B, 36 min M1 Max, train shard subsampled to 1500/task per documented amendment D-1) — own-substrate g_h8=-0.1093 fail-to-reject (Welch p=0.5992 below H7B threshold 0.5), cross-substrate vs G4-quinto Step 2 MLX small-CNN all arms reject H0 (g_mlx_minus_esnn ∈ [+1.21, +1.32]), g_p_equ_cross=+1.31 between H8-A floor 2.0 and H8-B ceiling 1.0 ; **H8-C classification = partial (architecture and LIF non-linearity both contribute)**, CNN architecture closes ~2/3 of cross-substrate level gap (G5-bis +4.02 → G5-ter +1.31 at P_equ) but cycle-3 positive effect remains absent on E-SNN regardless of architecture ; DR-3 evidence revised (axiom-test-level preserved, empirical effect-size transferability now refuted at two architectural depths) ; EC stays PARTIAL per pre-reg §11, FC stays C-v0.12.0. Prior : G5-bis (H7-B MLX-only artefact for the LIF MLP, +4.02 cross-substrate gap), G4-quinto (H5-C CONFIRMED, RECOMBINE-empty universalises across FMNIST + CIFAR-CNN), G4-quater (H4-C CONFIRMED on FMNIST 3-layer).
