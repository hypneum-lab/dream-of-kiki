"""E-SNN richer-head dream-episode wrapper for the G5-bis pilot.

Couples REPLAY+DOWNSCALE+RESTRUCTURE+RECOMBINE to the 3-layer LIF
classifier weights (``W_in``, ``W_h``, ``W_out``). The four step
functions are free functions taking the classifier as first arg ;
``dream_episode_hier_esnn`` orchestrates them following the same
P_min / P_equ / P_max op-set selection used in
``experiments.g4_ter_hp_sweep.dream_wrap_hier.dream_episode_hier``.

DR-0 accountability is preserved : ``profile.runtime.execute(episode)``
is called before any weight mutation so every episode appends one
``EpisodeLogEntry`` to ``profile.runtime.log`` regardless of
substrate-side outcome.

The profile is built via
``experiments.g5_cross_substrate.esnn_dream_wrap.build_esnn_profile``
(re-exported here as ``build_esnn_richer_profile``) so the
profile's op handlers dispatch through the E-SNN substrate's
typed factories — the same code path G5 used for the spectator
pattern test.
...
Reference :
    ...
    experiments/g5_cross_substrate/esnn_dream_wrap.py (G5 binary head)
"""
...
class BetaRecordHier(TypedDict, total=False):
    """One curated episodic exemplar for the LIF richer head."""
...
class EsnnHierBetaBuffer:
    """Bounded curated episodic buffer with optional latents.

    FIFO eviction at capacity. Mirrors
    ``BetaBufferHierFIFO`` from
    ``experiments.g4_ter_hp_sweep.dream_wrap_hier`` — same field
    shape and eviction policy. ``latent=None`` is allowed for
    pre-classifier-warmup pushes.
    """
...
def build_esnn_richer_profile(name: str, seed: int) -> ProfileT:
    ...
    cross-substrate dispatch path G5 binary head exercised. The
...
def _downscale_step(
    clf: EsnnG5BisHierarchicalClassifier, *, factor: float
) -> None:
    """Multiply ``W_in``, ``W_h``, ``W_out`` by ``factor``.
    ...
    clf.W_in = (clf.W_in * f32).astype(np.float32)
    clf.W_h = (clf.W_h * f32).astype(np.float32)
    clf.W_out = (clf.W_out * f32).astype(np.float32)
...
def _restructure_step(
    ...
    *,
    ...
    """Add ``factor * sigma * N(0, 1)`` noise to ``W_h`` only.

    ``sigma`` is the standard deviation of ``W_h`` at call time.
    ``factor=0`` is a no-op (early exit) ; ``factor < 0`` raises.
    Matches the framework C sec 3.1 RESTRUCTURE intuition that the
    middle hidden layer's connectivity is mutated without touching
    the input projection or the output classifier.
    """
    if factor < 0.0:
        raise ValueError(
    ...
    if factor == 0.0:
        return
    sigma = float(clf.W_h.std()) if clf.W_h.size > 0 else 0.0
    if sigma == 0.0:
        return
    rng = np.random.default_rng(seed)
    noise = rng.standard_normal(size=clf.W_h.shape).astype(np.float32)
    clf.W_h = (
        clf.W_h + np.float32(factor) * np.float32(sigma) * noise
    ).astype(np.float32)
...
def _recombine_step(
    ...
    *,
    ...
    """One CE-loss SGD step on ``W_out`` only over per-class MoG samples.
    ...
    run one softmax-CE SGD step on ``W_out`` (forward = synthetic
    rates @ W_out, backward = standard CE gradient). Leaves
    ``W_in`` and ``W_h`` untouched.
    """
    if not latents:
        return
    ...
    rng = np.random.default_rng(seed)
    components: dict[int, tuple[NDArray[np.float32], NDArray[np.float32]]] = {}
    for c in classes:
        arr = np.asarray(
        ...
        for _ in range(per_class):
            synth_x.append(
                (
                    mean
                    + std
                    * rng.standard_normal(size=mean.shape).astype(np.float32)
                ).astype(np.float32)
            )
            synth_y.append(c)
    ...
    logits = (rates @ clf.W_out).astype(np.float32)
    ...
    d_logits = ((probs - one_hot) / max(len(y_arr), 1)).astype(np.float32)
    d_W_out = rates.T @ d_logits
    clf.W_out = (
        clf.W_out - np.float32(lr) * d_W_out.astype(np.float32)
    ).astype(np.float32)
...
def dream_episode_hier_esnn(
    clf: EsnnG5BisHierarchicalClassifier,
    profile: ProfileT,
    seed: int,
    *,
    beta_buffer: EsnnHierBetaBuffer,
    ...
    """Drive one DreamEpisode and mutate classifier weights.

    Coupling map :
        - Operation.REPLAY    -> ``_replay_step`` over buffer sample
        - Operation.DOWNSCALE -> ``_downscale_step`` on (W_in, W_h, W_out)
        - Operation.RESTRUCTURE -> ``_restructure_step`` on W_h only
        - Operation.RECOMBINE -> ``_recombine_step`` on W_out only

    P_min runs REPLAY+DOWNSCALE only ; P_equ / P_max run all four ops.
    """
    ...
    # with the E-SNN handler factories' expectations (carries
    # "input" key for the replay handler).
    ...
    episode = DreamEpisode(
        ...
        episode_id=f"g5bis-{type(profile).__name__}-seed{seed}",
    )
    profile.runtime.execute(episode)

    # ---- G5-bis coupling on richer LIF substrate ----
